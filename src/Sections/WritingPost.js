	...
	return (
		<WritingPost className="section">
			...
			<div className="change-post">
				{post.index > 0 && (
					<div className="previous" onClick={() => loadNewPost(posts[post.index - 1].path)}>
						<div className="change-title">Previous</div>
						<div className="change-post-name">{posts[post.index - 1].title}</div>
					</div>
				)}
				{post.index < posts.length - 1 && (
					<div className="next" onClick={() => loadNewPost(posts[post.index + 1].path)}>
